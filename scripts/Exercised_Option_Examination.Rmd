...
# opt_preclean$qGrowth<-Hmisc::cut2(opt_preclean$p_SteadyScopeOptionGrowthAlone-1,c(1,10))
# summary(opt_preclean$qGrowth)
...
nrow(opt_preclean %>% filter((p_SteadyScopeOptionGrowthAlone-1)>1))
nrow(opt_preclean %>% filter((p_SteadyScopeOptionGrowthAlone-1)>10))
nrow(opt_preclean %>% filter((p_SteadyScopeOptionGrowthAlone-1)>100))
...
opt_preclean$Why_Outlier[opt_preclean$UnmodifiedBase<=0]<-"No Unmodified Base"
...
opt_preclean$Why_Outlier[is.na(opt_preclean$Why_Outlier)&
                            opt_preclean$Action_Obligation*2>=
                            (opt_preclean$SteadyScopeOptionGrowthAlone+opt_preclean$OtherCeilingModification)]<-
  "At Least 1/2 After Other Rescision"
opt_preclean$Why_Outlier[is.na(opt_preclean$Why_Outlier)&
                            opt_preclean$topContractingOfficeID=="W912UM"]<-
...
p_outlier_summary<-opt_preclean %>% filter(p_SteadyScopeOptionGrowthAlone-1>10) %>% group_by(Why_Outlier) %>%
  dplyr::summarise(nContract=length(SteadyScopeOptionGrowthAlone),
                   SumOfSteadyScopeOptionGrowthAlone=sum(SteadyScopeOptionGrowthAlone),
...
write.csv(file="..\\Data\\semi_clean\\p_OptGrowth_outliers.csv",opt_preclean %>% filter((p_SteadyScopeOptionGrowthAlone-1)>10),row.names = FALSE)
write.csv(file="..\\Data\\semi_clean\\n_OptGrowth_outliers.csv",opt_preclean %>% filter(SteadyScopeOptionGrowthAlone>=2.5e8),row.names = FALSE)
```
...
* Obligations at least half Orig+CRai: For this category, total obligations of the contract were at least half the value of the initial base plus options growth under change orders. These contrats have had spending that massively exceeded their original base, so the growth in absolute terrms seems plausible. This category accounts for the overwhelming majority of outlier spending but only a tiny fraction of change order growth.
...
(
ggplot(opt_preclean, aes(x=UnmodifiedBase+SteadyScopeOptionGrowthAlone,y=Action_Obligation_Then_Year)) +#,color=qp_OptGrowth
  geom_point(alpha=0.25,shape=".")+
  # theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_x_log10()+scale_y_log10()+
  #+
...
#+geom_vline(xintercept = 0.1)+
#facet_grid(NoPreTermObl~.,scales="free_y", space="free_y")+
   labs(title="Distribution of Contracts by Exercised Options Growth, Grouped by Base")#,
  #      y="Contract Count",
  #      x="Percent of Obligations After Day of Termination",
  #      fill="Termination Completion"
)
...
summary(opt_preclean$Ceil[(opt_preclean$p_SteadyScopeOptionGrowthAlone-1)>10 & opt_preclean$UnmodifiedBase>0 &
                            !is.na(opt_preclean$UnmodifiedBase>0 )])
summary(opt_preclean$Ceil[(opt_preclean$p_SteadyScopeOptionGrowthAlone-1)>100 & opt_preclean$UnmodifiedBase>0&
                            !is.na(opt_preclean$UnmodifiedBase>0)])
...
summary(opt_preclean$Ceil[opt_preclean$SteadyScopeOptionGrowthAlone>=1e6])
...
opt_preclean$Why_Outlier[is.na(opt_preclean$Why_Outlier)&
                   opt_preclean$Action_Obligation_Then_Year*2>=opt_preclean$UnmodifiedBase+
                   opt_preclean$SteadyScopeOptionGrowthAlone]<-
  "Obligations at least half Base+Opt"
opt_preclean$Why_Outlier[is.na(opt_preclean$Why_Outlier)&
                   opt_preclean$Office=="W912UM"]<-
  "Korean Office W912UM"
opt_preclean$Why_Outlier[is.na(opt_preclean$Why_Outlier)&((opt_preclean$UnmodifiedBase + opt_preclean$SteadyScopeOptionGrowthAlone) < opt_preclean$UnmodifiedCeiling_Then_Year)] <- "Base + Growth < Unmodified Ceiling"
opt_preclean$Why_Outlier[is.na(opt_preclean$Why_Outlier)&
                   opt_preclean$SteadyScopeOptionGrowthAlone>=2.5e8]<-
  ">=$250M, Inspect"
opt_preclean$Why_Outlier[is.na(opt_preclean$Why_Outlier)&
...
```
Examining cases of large options growth, `r nrow(opt_preclean %>% filter(p_SteadyScopeOptionGrowthAlone-1>10))` contracts experienced greater than 10 fold growth. An increase of that size strains credulity, even in high risk defense contracting. While by no means impossible, the more likely explaination is a misrecorded base.
...
#### >250 Inspect
```{r gt250m, echo=FALSE}
if(nrow(opt_preclean %>% filter(Why_Outlier==">=$250M, Inspect"))>0) stop("Contracts to inspect found")
...
inspect250trans<-left_join(inspect250  %>% 
                           ...
                           inspect250trans, 
                           by="CSIScontractID")
if(any(is.na(inspect250trans$piid))) stop(paste("Missing CSIScontractIDs in gt250k_n_exercised_opt_outliers.txt:",
...
p_inspect_summary<-inspect10x %>% group_by(CSIS_inspection) %>%
  dplyr::summarise(nContract=length(CSIScontractID),
    ...
    SteadyScopeOptionRescinded=sum(SteadyScopeOptionRescinded),
...
opt_preclean$qp_OptGrowth[opt_preclean$topContractingOfficeID=="W912UM"]<-"W912UM"
summary(opt_preclean$qp_OptGrowth)
...
write.csv(file="..\\Data\\semi_clean\\p_OptGrowth_outliers_post_cleaning.csv",opt_preclean %>% filter((p_SteadyScopeOptionGrowthAlone-1)>10),row.names = FALSE)
write.csv(file="..\\Data\\semi_clean\\n_OptGrowth_outliers_post_cleaning.csv",opt_preclean %>% filter(SteadyScopeOptionGrowthAlone>=2.5e8),row.names = FALSE)
```
...
opt_preclean$n_OptGrowth <- opt_preclean$SteadyScopeOptionGrowthAlone +
  ifelse(opt_preclean$SteadyScopeOptionRescision+opt_preclean$AdminOptionModification<0,
         opt_preclean$SteadyScopeOptionRescision+opt_preclean$AdminOptionModification,0)+1
...
if(any(!is.na(def_serv$UnmodifiedContractBaseAndExercisedOptionsValue[
...
cbre$qp_OptGrowth<-Hmisc::cut2(cbre$p_OptGrowth-1,c(1,10))
...
cbre$Why_Outlier<-NA
cbre$Why_Outlier[is.na(cbre$UnmodifiedBase_Then_Year)]<-"No Unmodified Base"
cbre$Why_Outlier[is.na(cbre$Why_Outlier)&
                   cbre$Action_Obligation_Then_Year*2>=cbre$UnmodifiedBase_Then_Year+
                   cbre$n_OptGrowth]<-
  "Obligations at least half Orig+CRai"
cbre$Why_Outlier[is.na(cbre$Why_Outlier)&
                   cbre$Office=="W912UM"]<-
  "Korean Office W912UM"
cbre$Why_Outlier[is.na(cbre$Why_Outlier)&
                   cbre$n_OptGrowth>=2.5e8]<-
  ">=$250M, Inspect"
cbre$Why_Outlier[is.na(cbre$Why_Outlier)&
                   cbre$p_OptGrowth-1>10]<-
  "Other Unexplained 10x Options Growth"
...
summary(Hmisc::cut2(cbre$p_OptGrowth-1,c(1,
                                    10,
                                    100
)))

summary(cbre$qHighBase[(cbre$p_OptGrowth-1)>10])
summary(cbre$qHighBase[(cbre$p_OptGrowth-1)>100])


p_outlier_summary<-cbre %>% filter(p_OptGrowth-1>10) %>% group_by(Why_Outlier) %>%
  dplyr::summarise(nContract=length(n_OptGrowth),
                   SumOfn_OptGrowth=sum(n_OptGrowth),
                   MaxOfn_OptGrowth=max(n_OptGrowth),
                   SumOfAction_Obligation_Then_Year=sum(Action_Obligation_Then_Year))

#Absolute Growth
summary(Hmisc::cut2(cbre$n_OptGrowth,c(1e3,
                                           1e6,
                                           1e7,
                                           1e8,
                                           2.5e8,
...
summary(cbre$qHighBase[cbre$n_OptGrowth>=1e6])
summary(cbre$qHighBase[cbre$n_OptGrowth>=1e9])



n_outlier_summary<-cbre %>% filter(n_OptGrowth>2.5e8) %>% group_by(Why_Outlier) %>%
  dplyr::summarise(nContract=length(n_OptGrowth),
                   SumOfn_OptGrowth=sum(n_OptGrowth),
                   MaxOfn_OptGrowth=max(n_OptGrowth),
                   SumOfAction_Obligation_Then_Year=sum(Action_Obligation_Then_Year))
...
`r knitr::kable(p_outlier_summary)`
* Obligations at least half Orig+CRai: For this category, total obligations of the contract were at least half the value of the initial base plus options growth under alone steady scope. As before, this category accounts for the overwhelming majority of outlier spending but only a tiny fraction of change order growth.
* There are `r nrow(cbre %>% dplyr::filter(Why_Outlier ==">=$250M, Inspect" ))` contracts with options growth of over $250 million that account for hundreds of billions in change order growth. These merit manual inspection.
...
(
  ggplot(cbre, aes(x=UnmodifiedBase_Then_Year,y=p_OptGrowth-1)) +#,color=qGrowth
    geom_point(alpha=0.25,shape=".")+
    # theme(axis.text.x = element_text(angle = 90, hjust = 1))+
    scale_x_log10()+scale_y_log10()+
    #+
...
(
  ggplot(cbre, aes(x=UnmodifiedBase_Then_Year,y=n_OptGrowth)) +#,color=qGrowth
    geom_point(alpha=0.25,shape=".")+
    # theme(axis.text.x = element_text(angle = 90, hjust = 1))+
    scale_x_log10()+scale_y_log10()+
    #+
...
(
  ggplot(cbre, aes(x=UnmodifiedBase_Then_Year+n_OptGrowth,
...
(
  ggplot(cbre, aes(x=n_OptGrowth,
                   y=Action_Obligation_Then_Year-UnmodifiedBase_Then_Year)) +#,color=qGrowth
    geom_point(alpha=0.25,shape=".")+
    # theme(axis.text.x = element_text(angle = 90, hjust = 1))+
    scale_x_log10()+scale_y_log10()#+
  #+
...
# 
(
  ggplot(cbre, aes(x=p_OptGrowth-1,
                   y=Action_Obligation_Then_Year/UnmodifiedBase_Then_Year)) +#,color=qGrowth
    geom_point(alpha=0.25,shape=".")+
    # theme(axis.text.x = element_text(angle = 90, hjust = 1))+
    scale_x_log10()+scale_y_log10()#+
...
(
  ggplot(cbre, aes(x=p_OptGrowth-1,fill=qGrowth)) +
    geom_histogram(bins=100)+
    theme(axis.text.x = element_text(angle = 90, hjust = 1))+
    scale_x_log10()+
    #+
...
(
  ggplot(cbre, aes(x=n_OptGrowth,fill=qGrowth)) +
    geom_histogram(bins=100)+
    theme(axis.text.x = element_text(angle = 90, hjust = 1))+
    scale_x_log10()+
    #+
