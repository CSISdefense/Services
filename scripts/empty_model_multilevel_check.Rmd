...
smp$CrisisProductOrServiceArea
if(!exists("cpsa_cbre"))
cpsa_cbre <- glmer(data=smp,
                 b_CBre ~ (1 | CrisisProductOrServiceArea)
                  , family=binomial(link="logit"))
get_icc(display=TRUE,cpsa_cbre)

if(!exists("cpsa_term"))
cpsa_term <- glmer(data=smp,
...
# 
# 
#   if("ProdServ" %in% colnames(smp)){
#     smp$ProdServ[smp$ProdServ==""]<-NA
#     smp$ProductOrServiceCode<-as.character(smp$ProdServ)
#   }
# 
#   if("ProductOrServiceCode" %in% colnames(smp)){
#     smp<-read_and_join( smp,
...
# 
#     smp$ProductServiceOrRnDarea<-factor(smp$ProductServiceOrRnDarea)
#     smp$ProductOrServiceArea<-factor(smp$ProductOrServiceArea)
#     smp$HostNation3Category<-factor(smp$HostNation3Category)
#     smp$CrisisProductOrServiceArea<-factor(smp$CrisisProductOrServiceArea)
#     smp$ProductOrServiceCodeText<-factor(smp$ProductOrServiceCodeText)
#     
  # }
...
```{r PSR}
smp$ServCommCons<-smp$HostNation3Category




if(!exists("ServCommCons_cbre"))
ServCommCons_cbre <- glmer(data=smp,
...
ServCommCons_term <- glmer(data=smp,
...
if(!exists("naics36_plus_cpsa_psc_term"))
  naics36_plus_cpsa_psc_term <- glmer(data=smp,
...
if(!exists("naics36_plus_cpsa_psc_cbre"))
  naics36_plus_cpsa_psc_cbre<- glmer(data=smp,
...
if(!exists("naics36_plus_cpsa_psc_comp"))
  naics36_plus_cpsa_psc_comp <- glmer(data=smp,
...
### NAICS 3/6 / CPSA /PSC
```{r NAICSlevel36_CPSA_psc}

if(!exists("naics36_cpsa_psc_term"))
  naics36_cpsa_psc_term<- glmer(data=smp,
                        b_Term ~ (1 | NAICS3/NAICS/CrisisProductOrServiceArea/ProdServ)
                        , family=binomial(link="logit"))
get_icc(display=TRUE,naics36_cpsa_psc_term)

if(!exists("naics36_cpsa_psc_cbre"))
  naics36_cpsa_psc_cbre<- glmer(data=smp,
                        b_CBre ~ (1 | NAICS3/NAICS/CrisisProductOrServiceArea/ProdServ)
                        , family=binomial(link="logit"))
get_icc(display=TRUE,naics36_cpsa_psc_cbre)

if(!exists("naics36_cpsa_psc_comp"))
  naics36_cpsa_psc_comp <- glmer(data=smp,
                        b_Comp ~ (1 | NAICS3/NAICS/CrisisProductOrServiceArea/ProdServ)
                        , family=binomial(link="logit"))
get_icc(display=TRUE,naics36_cpsa_psc_comp)
```
...
get_icc(naics36_cpsa_cbre)
      ...
      stargazer::stargazer(naics36_cbre,
                           cpsa_psc_cbre,naics36_plus_cpsa_psc_cbre,
                           naics36_cpsa_cbre,type="text",
                     digits=2)
```
...
get_icc(naics36_cpsa_term)
      ...
      stargazer::stargazer(naics36_term,
                           cpsa_psc_term,naics36_plus_cpsa_psc_term,
                           naics36_cpsa_term,type="text",
                     digits=2)
```
...
save(file="../data//clean//naics6_when_office.rdta",naics06_When_Office_term,naics06_When_Office_cbre)
```
...
save(b_Comp_naics36_Office_cbre,b_Comp_naics36_Office_term,
     ...
     file="../data//clean//comp_metric_only_models.rdata")
