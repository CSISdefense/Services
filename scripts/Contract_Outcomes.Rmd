...
For terminations expectation were upheld or S-IDCs and BPA/BOA. They werenot upheld for multi-award, wich is significantly more likely to be terminated.
...
```
Expectations for vehicle are now upheld for both ceining breaches and terminations. For other variables the addition of vehicle proved less pivotal, though coefficients were often decreased.
...
Other fixed price and other cost baed aligned with expecations for ceiling breaches.

FOr terminations, other fixed price, incentive, cost-based, were line with expectations. For options exercied Other fixed-price, incentive, and UCA were in line with expectations
...
For ceiling breaches ARRA and Disaster results were in keeping with expcetations but OCO results were not. There were no significant results for terminations. For options, contrary to exepectation, all forms of crii have a lower rate of usage.
...
summary_residual_compare(CBre_08B,CBre_09A,Term_08A,Term_09A)
...
Aligns with expecationss on terminatio, but not for ceiling  breach or for options.
...
CBre_12C <- glm (data=serv_smp,
                 ...
                 c_pairHist+cl_pairCA +
                   ...
                   cl_def3_HHI_lag1+cl_def3_ratio_lag1+
                   c_pMarket+
                   cl_OffVol, family=binomial(link="logit"))
