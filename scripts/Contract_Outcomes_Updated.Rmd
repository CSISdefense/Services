...
ggplot(serv_opt, aes(x=cl_US6_avg_sal_lag1, y=l_OptGrowth)) + geom_point() + ggtitle('Exercised Options') + theme(plot.title = element_text(hjust = 0.5))
...
summary_residual_compare(CBre_01A,Term_01A,bins=3, skip_vif = TRUE)
...
#Scatter Plot 
ggplot(serv_opt, aes(x=cl_CFTE, y=l_OptGrowth)) + geom_point() + ggtitle('Exercised Options') + theme(plot.title = element_text(hjust = 0.5))
...
summary_residual_compare(CBre_01B,Term_01B,bins=3, skip_vif = TRUE)
summary_residual_compare(CBre_01A,CBre_01B,Term_01A,Term_01B,bins=3, skip_vif = TRUE)
...
Both average salary and invoiced rate don't have large vif, suggesting that the variance of the estimated coefficients is not evidently inflated and none of them are highly correlated with each other. 
...
#Scatter Plot 
ggplot(serv_opt, aes(x=c_pPBSC, y=l_OptGrowth)) + geom_point() + ggtitle('Exercised Options') + theme(plot.title = element_text(hjust = 0.5))
...
#Scatter Plot 
ggplot(serv_opt, aes(x=c_pOffPSC, y=l_OptGrowth)) + geom_point() + ggtitle('Exercised Options') + theme(plot.title = element_text(hjust = 0.5))
...
summary_residual_compare(CBre_02A,CBre_02B,Term_02A,Term_02B,bins=3, skip_vif = TRUE)
...
#Scatter Plot
ggplot(serv_opt, aes(x=c_pairHist, y=l_OptGrowth)) + geom_point() + ggtitle('Exercised Options') + theme(plot.title = element_text(hjust = 0.5))
...
#Scatter Plot
ggplot(serv_opt, aes(x=cl_pairCA, y=l_OptGrowth)) + geom_point() + ggtitle('Exercised Options') + theme(plot.title = element_text(hjust = 0.5))
...
When considering contract actions alone, expectation was met for exercised options, but not for ceiling breaches and terminations.

### 03C: Vendor Market Share
Expectation:
...
#Scatter Plot
ggplot(serv_opt, aes(x=c_pMarket, y=l_OptGrowth)) + geom_point() + ggtitle('Exercised Options') + theme(plot.title = element_text(hjust = 0.5))
...
### 03D: Office-Vendor Relationship
Expectation: 
...
#Plot residuals versus fitted
stargazer::stargazer(CBre_02D,CBre_03A,CBre_03B,CBre_03D,
                       Term_02D,Term_03A,Term_03B,Term_03D,
                       Opt_02D,Opt_03A,Opt_03B,Opt_03D,
                       type="text",
                       digits=2)



summary_residual_compare(CBre_03A,CBre_03D,Term_03A,Term_03D)
summary_residual_compare(CBre_03B,CBre_03D,Term_03B,Term_03D)
...
1. Ceiling breaches:
   A. Service Complexity:
      The relationship with average salary lost significance.
      The result for invoice rate did not match with expectation.
   ...
   C. Office-Vendor Relationship:
      ...
      The relationship with contract actions lost significance.

2. Terminations:
   A. Service Complexity:
      The reult for average salary did not match with expectation.
      The result for invoice rate did matach with expectation but only significant (for p-value <0.1)
...
3. Exercised Options:
  A. Service Complexity:
      The reult for average salary matched with expectation all the time.
   ...
   C. Office-Vendor Relationship:
      The result for pair history did not match with expectation.
      The result for contract actions matched with expectation.
...
### Past Office Volume (dollars)

### Detailed Industry Diveristy
