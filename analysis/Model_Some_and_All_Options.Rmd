...
The results are negative and significant, meeting expectations for both variables, as higher average salaries estimates a lower probability of exercised options. 
...
When considered alone, expectations are matched for some and all options, as higher invoice rate estimates a significantly lower likelihood that some or all options will be exercised. 
...
summary_residual_compare(b_SomeOpt01A,b_SomeOpt01C,b_SomeOpt01B,b_SomeOpt01C,
                         b_AllOpt01A,b_AllOpt01C,b_AllOpt01B,b_AllOpt01C,bins=3)
```
...
For the some options model, combining the two inputs leaves only the invoice rate as a significant estimator of the likelihood of some options being exercised, though that result remains in line with expections. By AIC, including salary does not improve the model.

Both individually and pair-wise, higher average salary and invoiced rate estimate lower likelihood that all options will be exercised. 
...
When considering PBSC alone, the significant negative results were contrary to expectations for both some and all options.
...
When considering number of contracting office obligations for a given service alone, no result was found for some options and a significant negative relationship was found for the likelihood all options would be exercised given that some where. This is contrary to expectations.
...
summary_residual_compare(b_SomeOpt02A,b_SomeOpt02C,
                         b_SomeOpt02A,b_AllOpt02B)


```
For the some options model, combining the two variables causes Office PSC experience to becoming significant with a positive sign, in line with expectations, while office performance-based remains significant with a negative sign, contrary to expectations. For all options, service code experience remains significant and negative, contrary to expectations, and performance-based loses significance.
...
For some options, there are some shifts in coefficient, but the signs and significance all match the two prior cumulative model, with invoice rate significantly negative and Serv. Code experience significantly positvie in line with expectations but with perform-based negative and contrary to expectations.

For all options, salary is now significantly positive and invoice rate loses significance. No variables are presently significant in line with expectations.
...
When considering pair history alone, expectations were not met by the significant negative coefficients for both p(some option) and p(all options|some option).
...
Contrary to expectations paired actions estimates significant negative coefficients for both p(Some Options) and p(All Options | Some Options) 
