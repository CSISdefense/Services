...
Term_04E <- glm (data=serv_smp,
                 b_Term ~ cl_Base, family=binomial(link="logit"))
...
Term_04F <- glm (data=serv_smp,
                 b_Term ~ cl_Base +cl_Days+cl_Base2Ceil, family=binomial(link="logit"))
...
Term_04G <- glm (data=serv_smp,
                 b_Term ~ cl_US6_avg_sal_lag1Const + 
                   ...
                   cl_Base + cl_Days+cl_Base2Ceil, family=binomial(link="logit"))
...
```{r}
summary_continuous_plot(serv_smp1m,"cl_Base_Then_Year","PricingUCA",metric="cbre")
...
```{r}
summary_continuous_plot(serv_smp1m,"c_pPBSC","cl_pairCA",metric="cbre")
...
```{r}
summary_double_continuous(serv_smp1m,"c_pPBSC","c_pMarket")
...
```{r}
summary_double_continuous(serv_smp1m,"c_pPBSC","cl_Days")
...
Combination				+	-	-
```{r}
summary_discrete_plot(serv_smp1m,"c_pairHist","PricingUCA")
...
```{r}
summary_continuous_plot(serv_smp1m,"cl_def6_HHI_lag1","Comp1or5",metric="cbre")
...
```{r}
summary_double_continuous(serv_smp1m,"cl_Base2Ceil","cl_Base")

#Create the models
term_base2ceil_ceiling_21A_no_interact <- glm(data=serv_smp, b_Term ~ cl_Base2Ceil + cl_Base,
                 family=binomial(link="logit"))
glmer_examine(term_base2ceil_ceiling_21A_no_interact)

term_base2ceil_ceiling_21A <- glm(data=serv_smp, b_Term ~ cl_Base2Ceil + cl_Base + cl_Base2Ceil:cl_Base,
                 family=binomial(link="logit"))
...
```{r Model22A}
summary_double_continuous(serv_smp1m,"cl_Base2Ceil","cl_Days")
