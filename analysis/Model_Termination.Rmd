...
summary_continuous_plot(serv_smp1m,"cl_CFTE")
...
Contract ceiling has a significant relationship, though with a bit of a bump in the fitted curve.
...
summary_continuous_plot(serv_smp1m,"cl_Days")
...
```{r}
summary_continuous_plot(serv_smp1m, "cl_Base2Ceil")

#Scatter Plot
ggplot(serv_smp, aes(x=cl_Base2Ceil, y=b_Term)) + geom_point(alpha = 0.1) + ggtitle("Terminations and Ratio Initial Base:Ceiling Ratio (Logged") + theme(plot.title = element_text(hjust = 0.5))
...
stargazer::stargazer(Term_12C, Term_04C, type="text", digits=2)
```
...
#Plot residuals versus fitted
stargazer::stargazer(
                       Term_03D,Term_04A,Term_04B,Term_04D,
...
#summary_residual_compare(Term_04A,Term_04D)
#summary_residual_compare(Term_04B,Term_04D)
...
summary_residual_compare(Term_04A,Term_04D,Term_04B,Term_04D)
...
ggplot(glm(b_Term~Veh, data=serv_smp)) + geom_point(aes(x=.fitted, y=.resid)) + ggtitle('Residual Plot') + theme(plot.title = element_text(hjust = 0.5))
...
summary_continuous_plot(serv_smp1m,"cl_def6_HHI_lag1")
summary_continuous_plot(serv_smp1m,"cl_def3_HHI_lag1")
...
#Frequency Plot for unlogged ceiling
      ...
      summary_continuous_plot(serv_smp1m,"cl_def6_ratio_lag1")
...
#Frequency Plot for unlogged ceiling
      ...
      summary_continuous_plot(serv_smp1m,"cl_def6_obl_lag1Const")
...
summary_continuous_plot(serv_smp1m,"cl_def3_ratio_lag1")
...
summary_residual_compare(Term_10D)
...
summary_continuous_plot(serv_smp1m,"c_pMarket")
...
summary_continuous_plot(serv_smp1m,"cl_OffVol")
