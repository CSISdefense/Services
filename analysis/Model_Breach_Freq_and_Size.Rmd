...
####23A: CFTE (logged):Contract Pricing
Cost-based and incentive contract types may do a  better job at handling complex contracts.

```{r Model23A}
#Create the models
b_cbre_CFTE_pricing_23A_no_interact <- glm(data=serv_smp, b_CBre ~ PricingUCA + cl_CFTE, 
                                             family=binomial(link="logit"))
glmer_examine(b_cbre_CFTE_pricing_23A_no_interact)

b_cbre_CFTE_pricing_23A <- glm(data=serv_smp, b_CBre ~ PricingUCA + cl_CFTE + PricingUCA:cl_CFTE,
                                 family=binomial(link="logit"))
glmer_examine(b_cbre_CFTE_pricing_23A)


n_cbre_CFTE_pricing_23A_no_interact <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ PricingUCA + cl_CFTE)
glmer_examine(n_cbre_CFTE_pricing_23A_no_interact)

n_cbre_CFTE_pricing_23A <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ PricingUCA + cl_CFTE + PricingUCA:cl_CFTE)
glmer_examine(n_cbre_CFTE_pricing_23A)


#compare the models

stargazer::stargazer(b_cbre_CFTE_pricing_23A_no_interact, b_cbre_CFTE_pricing_23A, 
...
stargazer::stargazer(n_cbre_salary_pricing_15A_no_interact, n_cbre_salary_pricing_15A, 
                     n_cbre_CFTE_pricing_23A_no_interact, n_cbre_CFTE_pricing_23A, type="text", digits=2)

summary_residual_compare(b_cbre_CFTE_pricing_23A_no_interact, b_cbre_CFTE_pricing_23A, bins=5)
summary_residual_compare(n_cbre_CFTE_pricing_23A_no_interact, n_cbre_CFTE_pricing_23A, bins=3)
...
```{r Model23B}
#Create the model
b_cbre_CFTE_pricing_23B <- glm(data=serv_smp,
                        b_CBre ~  cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA+
                 cl_Base + cl_Base2Ceil + cl_Days+
                 ...
                 c_pPBSC:cl_Days+
                   cl_Base2Ceil:cl_Base+
                 PricingUCA:cl_CFTE, family=binomial(link="logit"))
glmer_examine(b_cbre_CFTE_pricing_23B)

n_cbre_CFTE_pricing_23B <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA+
                 cl_Base + cl_Base2Ceil + cl_Days+
                 Comp1or5+
                 ...
                 #cl_office_naics_hhi_k+
                   PricingUCA:cl_CFTE)
glmer_examine(n_cbre_CFTE_pricing_23B)

n_cbre_CFTE_pricing_23B2 <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA+
                 cl_Base + cl_Base2Ceil + cl_Days+
                 Comp1or5+
                 ...
                 #cl_office_naics_hhi_k+
                   PricingUCA:cl_CFTE)
glmer_examine(n_cbre_CFTE_pricing_23B2)

#Compare the models
  stargazer::stargazer(b_CBre_base2ceil_ceiling_21B, b_cbre_CFTE_pricing_23B, 
                       n_cbre_partnership_actions_16B, n_cbre_CFTE_pricing_23B2, type="text", digits=2)

summary_residual_compare(b_CBre12C3, b_cbre_CFTE_pricing_23B)
summary_residual_compare(n_cbre_partnership_actions_16B, n_cbre_CFTE_pricing_23B)
...
For size, VIF again precludes multiple interactions. c_PBSC:c_pMarket more closely aligns with expectations and has a lower AIC, maing keeping it an easy choice.
...
####27A: 2019-08-20 First run, original multilevel.
This was created while we were still using ceiling instead of base.
...
```{r Term27A}
#Create the model
b_CBre27A <- glmer(data=serv_smp, b_CBre ~  cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA+
                 cl_Base + cl_Base2Ceil + cl_Days+
                 Comp1or5+
...
#Compare the models
glmer_examine(n_CBre27A,display = TRUE)

save(b_CBre27A,file="..\\output\\CBre27A.rda")

stargazer::stargazer(b_CBre_base2ceil_ceiling_21B, b_CBre27A, 
                     n_cbre_partnership_actions_16B, n_CBre27A, type="text", digits=2)


summary_residual_compare( term_base2ceil_ceiling_21B, term27A,bins=100)
```
