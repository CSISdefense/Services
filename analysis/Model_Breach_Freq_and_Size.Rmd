...
(option,breach,termination)
Interact: Office Volume cl_OffVol: c_pMarket		+	+	-
...
b_cbre17C_no_interact <- glm(data=serv_smp, b_CBre ~ cl_OffVol + c_pMarket, family=binomial(link="logit"))
glmer_examine(b_cbre17C_no_interact)

b_cbre17C <- glm(data=serv_smp, b_CBre ~ cl_OffVol + c_pMarket + cl_OffVol:c_pMarket, family=binomial(link="logit"))
glmer_examine(b_cbre17C)

n_cbre17C_no_interact <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ cl_OffVol + c_pMarket)
glmer_examine(n_cbre17C_no_interact)

n_cbre17C <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ cl_OffVol + c_pMarket + cl_OffVol:c_pMarket)
...
b_cbre17D <- glm(data=serv_smp,
                 ...
                 cl_office_naics_hhi_k+
                 c_pPBSC:cl_pairCA+
                   c_pPBSC:c_pMarket+cl_OffVol:c_pMarket, family=binomial(link="logit"))
glmer_examine(b_cbre17D)


n_cbre17D <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA+
                 cl_Base + cl_Base2Ceil + cl_Days+
                 Comp1or5+
...
n_cbre17D2 <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA+
                 cl_Base + cl_Base2Ceil + cl_Days+
                 Comp1or5+
...
Significant negative estimate of likelihood is contrary to expectation. Significant positive estimate of breach size is in line with expectations but to avoid multicollinearity requires removing c_pPBSC:cl_pairCA. However, the model with c_pPBSC:cl_pairCA has a lower AIC and is relevant  to  a study variable. Leaving this interaction out.
...
For size, VIF again precludes multiple interactions. c_PBSC:c_pMarket more closely aligns with expectations and has a lower AIC, making keeping it an easy choice.

Current cumulative b_CBre model: b_CBre_base2ceil_ceiling_21B
Current cumulative n_Cbre model: n_cbre_partnership_actions_16B

####24A: Office Serv. Code Exp:Office Obligations
...
```{r Model24A}
#Create the models
b_cbre24A_no_interact <- glm(data=serv_smp, b_CBre ~ PricingUCA + cl_CFTE, 
                                             family=binomial(link="logit"))
glmer_examine(b_cbre24A_no_interact)

b_cbre24A <- glm(data=serv_smp, b_CBre ~ PricingUCA + cl_CFTE + PricingUCA:cl_CFTE,
                                 family=binomial(link="logit"))
glmer_examine(b_cbre24A)


n_cbre24A_no_interact <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ PricingUCA + cl_CFTE)
glmer_examine(n_cbre24A_no_interact)

n_cbre24A <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ PricingUCA + cl_CFTE + PricingUCA:cl_CFTE)
glmer_examine(n_cbre24A)


#compare the models

stargazer::stargazer(b_cbre24A_no_interact, b_cbre24A, 
                     b_cbre_salary_pricing_15A_no_interact, b_cbre_salary_pricing_15A, 
                     type="text", digits=2)
stargazer::stargazer(n_cbre_salary_pricing_15A_no_interact, n_cbre_salary_pricing_15A, 
                     n_cbre24A_no_interact, n_cbre24A, type="text", digits=2)

summary_residual_compare(b_cbre24A_no_interact, b_cbre24A, bins=5)
summary_residual_compare(n_cbre24A_no_interact, n_cbre24A, bins=3)
...
n_cbre24B <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA+
                 cl_Base + cl_Base2Ceil + cl_Days+
                 Comp1or5+
                 ...
                 #cl_office_naics_hhi_k+
                   PricingUCA:cl_CFTE)
glmer_examine(n_cbre24B)

n_cbre24B2 <- glm(data=serv_breach, ln_CBre_OMB20_GDP18 ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA+
                 cl_Base + cl_Base2Ceil + cl_Days+
                 Comp1or5+
                 ...
                 #cl_office_naics_hhi_k+
                   PricingUCA:cl_CFTE)
glmer_examine(n_cbre24B2)

#Compare the models
  stargazer::stargazer(b_CBre_base2ceil_ceiling_21B, b_cbre24B, 
                       n_cbre_partnership_actions_16B, n_cbre24B2, type="text", digits=2)

summary_residual_compare(b_CBre12C3, b_cbre24B)
summary_residual_compare(n_cbre_partnership_actions_16B, n_cbre24B)
...
Dropping c_pPBSC:c_pMarket and c_pPBSC:c_pMarket as they were not significant in 2A. Adding PricingUCA:cl_CFTE as a realized that it is another way to consider PricingUCA:AverageSalary that turns out to avoid VIF problems in the likelihood of breach model. 

Current cumulative b_CBre model: b_cbre_CFTE_pricing_23B, b_CBre25A
Current cumulative n_Cbre model: n_cbre_partnership_actions_16B, n_CBre25A
...
stargazer::stargazer(b_cbre_CFTE_pricing_23B, b_CBre25A, b_CBre27A, type="text", digits=2)


summary_residual_compare(b_cbre_CFTE_pricing_23B, b_CBre27A, b_CBre25A, b_CBre27A,bins=100)
...
Trimming down to interactions that are present in n_CBRre (pPBSC:cl_pairCA) or significant and in line with expectations (pPBSC:Days)
...
####29A: 2019-08-22 Restoring cl_def3_HHI_lag1 and cl_office_naics_hhi_k
...
if(!exists("b_CBre29B")){
  #Create the model
  b_CBre29A <- glmer(data=serv_smp, b_CBre ~  cl_US6_avg_sal_lag1Const +
                       cl_CFTE+ c_pPBSC+c_pOffPSC+
                       c_pairHist+cl_pairCA+
                       cl_Base + cl_Base2Ceil + cl_Days+
                       Comp1or5+
  ...
  b_CBre29B <- glmer(data=serv_smp1m, b_CBre ~  cl_US6_avg_sal_lag1Const +
                       cl_CFTE+ c_pPBSC+c_pOffPSC+
                       c_pairHist+cl_pairCA+
                       cl_Base + cl_Base2Ceil + cl_Days+
                       Comp1or5+
...
stargazer::stargazer(b_CBre25A, b_CBre28A, b_CBre29A, type="text", digits=2)

summary_residual_compare( b_CBre25A, b_CBreA,b_CBre25A,b_CBreB,bins=100)
```