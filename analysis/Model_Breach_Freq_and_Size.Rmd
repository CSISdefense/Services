...
if(!"ln_CBre" %in% colnames(serv_smp))
  colnames(serv_smp)[colnames(serv_smp)=="ln_CBre_OMB20_GDP18"]<-"ln_CBre"

if(!"ln_CBre" %in% colnames(serv_smp1m))
  colnames(serv_smp1m)[colnames(serv_smp1m)=="ln_CBre_OMB20_GDP18"]<-"ln_CBre"

if(!"ln_CBre" %in% colnames(serv_breach))
  colnames(serv_breach)[colnames(serv_breach)=="ln_CBre_OMB20_GDP18"]<-"ln_CBre"

memory.limit(64000)


contract_transform_verify(serv_smp1m,just_check_na = TRUE)
contract_transform_verify(serv_smp,just_check_na = TRUE)
contract_transform_verify(serv_breach,just_check_na = TRUE)
...
n_cbre_salary_pricing_15A_no_interact <- glm(data=serv_breach, ln_CBre ~ PricingUCA + cl_US6_avg_sal_lag1)
glmer_examine(n_cbre_salary_pricing_15A_no_interact)

n_cbre_salary_pricing_15A <- glm(data=serv_breach, ln_CBre ~ PricingUCA + cl_US6_avg_sal_lag1 + PricingUCA:cl_US6_avg_sal_lag1)
...
```{r Model19A}
#Create the models
b_cbre_history_pricing_19A_no_interact <- glm(data=serv_smp, b_CBre ~ cn_PairHist7 + PricingUCA, family=binomial(link="logit"))
glmer_examine(b_cbre_history_pricing_19A_no_interact)

b_cbre_history_pricing_19A <- glm(data=serv_smp, b_CBre ~ cn_PairHist7 + PricingUCA + cn_PairHist7:PricingUCA, family=binomial(link="logit"))
glmer_examine(b_cbre_history_pricing_19A)

n_cbre_history_pricing_19A_no_interact <- glm(data=serv_breach, ln_CBre ~ cn_PairHist7 + PricingUCA)
glmer_examine(n_cbre_history_pricing_19A_no_interact)

n_cbre_history_pricing_19A <- glm(data=serv_breach, ln_CBre ~ cn_PairHist7 + PricingUCA + cn_PairHist7:PricingUCA)
...
```{r Model22A}
#Create the models
b_cbre_CFTE_pricing_22A_no_interact <- glm(data=serv_smp, b_CBre ~ PricingUCA + cln_PSCrate, 
                                             family=binomial(link="logit"))
glmer_examine(b_cbre_CFTE_pricing_22A_no_interact)

b_cbre_CFTE_pricing_22A <- glm(data=serv_smp, b_CBre ~ PricingUCA + cln_PSCrate + PricingUCA:cln_PSCrate,
                                 family=binomial(link="logit"))
glmer_examine(b_cbre_CFTE_pricing_22A)


n_cbre_CFTE_pricing_22A_no_interact <- glm(data=serv_breach, ln_CBre ~ PricingUCA + cln_PSCrate)
glmer_examine(n_cbre_CFTE_pricing_22A_no_interact)

n_cbre_CFTE_pricing_22A <- glm(data=serv_breach, ln_CBre ~ PricingUCA + cln_PSCrate + PricingUCA:cln_PSCrate)
...
if(!exists("b_CBre29B")){
  ...
  n_CBre29A <- lmer(data=serv_breach, ln_CBre ~ cln_US6sal + 
                      cln_PSCrate+ cp_OffPerf7+cp_OffPSC7+
                      cn_PairHist7+cln_PairCA+
                      cln_Base + clr_Ceil2Base + cln_Days+
                      Comp+
                      ...
                      (1 | StartFY),
                    verbose=TRUE)
  glmer_examine(n_CBre29A,display = TRUE)
  save(b_CBre29A,b_CBre29B,n_CBre29A,file="..\\output\\b_CBre29AB.rda")
}
...
stargazer::stargazer(n_cbre_partnership_actions_16B, n_CBre25A,n_CBre26A, n_CBre29A,type="text", digits=2)
...
Adding off focus is significant in its own right in the expected direction and lowers AIC and BIC. Slightly raises AIC and BIC for n_CBre, though clm_Def3HHI is significant and in the expected direction. Seems worth it for consistency.

### 29C Adding PBSC:Days to n_CBre
...
if(!exists("n_CBre29C")){
  ...
  n_CBre29C <- lmer(data=serv_breach, ln_CBre ~ cln_US6sal + 
                      ...
                      (1 | StartFY),
                    verbose=TRUE)
  glmer_examine(n_CBre29C,display = TRUE)
...
stargazer::stargazer(n_cbre_partnership_actions_16B, n_CBre25A,n_CBre26A, n_CBre29A,n_CBre29C,type="text", digits=2)


summary_residual_compare( n_CBre25A,n_CBre29C,
                          n_CBre29A, n_CBre29C,bins=100)
