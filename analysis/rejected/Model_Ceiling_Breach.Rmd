...
#Model
n_CBre13A <- glm(data=serv_smp, ln_CBre ~ cl_Base2Ceil)
display(n_CBre13A)
...
```{r}
#Create the models
n_cbre_salary_pricing_15A_no_interact <- glm(data=serv_smp, ln_CBre ~ PricingUCA + cl_US6_avg_sal_lag1)
glmer_examine(n_cbre_salary_pricing_15A_no_interact)

n_cbre_salary_pricing_15A <- glm(data=serv_smp, ln_CBre ~ PricingUCA + cl_US6_avg_sal_lag1 + PricingUCA:cl_US6_avg_sal_lag1)
...
```{r}
#Create the model
n_cbre_salary_pricing_15B <- glm(data=serv_smp, ln_CBre ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA +
                   cl_Ceil + capped_cl_Days+
                   Comp1or5+
...
```{r}
#Create the models
n_cbre_partnership_actions_16A_no_interact <- glm(data=serv_smp, ln_CBre ~ c_pPBSC + cl_pairCA)
glmer_examine(n_cbre_partnership_actions_16A_no_interact)

n_cbre_partnership_actions_16A <- glm(data=serv_smp, ln_CBre ~ c_pPBSC + cl_pairCA + c_pPBSC:cl_pairCA)
...
```{r}
#Create the model
n_cbre_partnership_actions_16B <- glm(data=serv_smp, ln_CBre ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA +
                   cl_Ceil + capped_cl_Days+
                   Comp1or5+
...
```{r}
#Define the models
n_cbre_partnership_share_17A_no_interact <- glm(data=serv_smp, ln_CBre ~ c_pPBSC + c_pMarket)
glmer_examine(n_cbre_partnership_share_17A_no_interact)

n_cbre_partnership_share_17A <- glm(data=serv_smp, ln_CBre ~ c_pPBSC + c_pMarket + c_pPBSC:c_pMarket)
...
```{r}
#Create the model
n_cbre_partnership_share_17B <- glm(data=serv_smp, ln_CBre ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA +
                   cl_Ceil + capped_cl_Days+
                   Comp1or5+
...
n_cbre_partnership_duration_18A <- glm(data=serv_smp, ln_CBre ~ c_pPBSC + capped_cl_Days + c_pPBSC: capped_cl_Days)
...
```{r}
#Create the model
n_cbre_partnership_duration_18B <- glm(data=serv_smp, ln_CBre ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA +
                   cl_Ceil + capped_cl_Days+
                   Comp1or5+
...
```{r}
#Create the models
n_cbre_history_pricing_19A_no_interact <- glm(data=serv_smp, ln_CBre ~ c_pairHist + PricingUCA)
glmer_examine(n_cbre_history_pricing_19A_no_interact)

n_cbre_history_pricing_19A <- glm(data=serv_smp, ln_CBre ~ c_pairHist + PricingUCA + c_pairHist:PricingUCA)
...
```{r}
#Create the model
n_cbre_history_pricing_19B <- glm(data=serv_smp, ln_CBre ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA +
                   cl_Ceil + capped_cl_Days+
                   Comp1or5+
...
```{r}
#Create the models
n_cbre_competition_naics_20A_no_interact <- glm(data=serv_smp, ln_CBre ~ Comp1or5 + cl_def6_HHI_lag1)
glmer_examine(n_cbre_competition_naics_20A_no_interact)

n_cbre_competition_naics_20A <- glm(data=serv_smp, ln_CBre ~ Comp1or5 + cl_def6_HHI_lag1 + Comp1or5:cl_def6_HHI_lag1)
...
```{r}
#Create the model
n_cbre_competition_naics_20B <- glm(data=serv_smp, ln_CBre ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA +
                   cl_Ceil + capped_cl_Days+
                   Comp1or5+
...
```{r}
#Create the models
n_CBre_base2ceil_ceiling_21A_no_interact <- glm(data=serv_smp, ln_CBre ~ cl_Base2Ceil + cl_Ceil)
glmer_examine(n_CBre_base2ceil_ceiling_21A_no_interact)

n_CBre_base2ceil_ceiling_21A <- glm(data=serv_smp, ln_CBre ~ cl_Base2Ceil + cl_Ceil + cl_Base2Ceil:cl_Ceil)
...
```{r}
#Create the model
n_CBre_base2ceil_ceiling_21B <- glm(data=serv_smp, ln_CBre ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA +
                   cl_Ceil + capped_cl_Days+
                   Comp1or5+
...
```{r}
n_CBre_base2ceil_duration_22A_no_interact <- glm(data=serv_smp, ln_CBre ~ cl_Base2Ceil + capped_cl_Days)
glmer_examine(n_CBre_base2ceil_duration_22A_no_interact)

n_CBre_base2ceil_duration_22A <- glm(data=serv_smp, ln_CBre ~ cl_Base2Ceil + capped_cl_Days + cl_Base2Ceil:capped_cl_Days)
...
```{r}
#Create the model
n_CBre_base2ceil_duration_22B <- glm(data=serv_smp, ln_CBre ~ cl_US6_avg_sal_lag1Const + 
                   cl_CFTE+ c_pPBSC+c_pOffPSC+
                 c_pairHist+cl_pairCA +
                   cl_Ceil + capped_cl_Days+
                   Comp1or5+
