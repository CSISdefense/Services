...
```{r Options}
grouped_barplot("OptExer", def_serv)
...
statsummary_discrete("AnyUnmodifiedUnexercisedOptions", def_serv)
...
statsummary_continuous(c("n_CBre_OMB20_GDP18"), def_serv %>% dplyr::filter(b_CBre==1),digits=0)
```
...
statsummary_continuous(c("US6_avg_sal_lag1Const","CFTE_Rate_1year"), def_serv, digits = 0)
```
...
statsummary_continuous(c("pPBSC","pOffPSC","office_entity_paircount_7year"), def_serv,log=FALSE, digits=3)
...
statsummary_continuous(c("office_entity_numberofactions_1year"), def_serv, plus1=TRUE,digits=1)
```
...
```{r Competition}  
freq_continuous_plot(def_serv %>% dplyr::filter(Comp!="No Comp."), #Only Competed
...
scope<-statsummary_continuous(c("UnmodifiedBase_OMB20_GDP18","Ceil2Base","UnmodifiedDays"), def_serv)
...
grouped_barplot("AgencyIDtext", def_serv,top_rank = 5)

grouped_barplot("AgencyIDtext", def_serv,top_rank = 10)
...
#get the detailed desceription for Office by Agency and Office
...
# def_dup <- def_serv
#Generate a new name column by Combining officeID and ContractingOfficeName

def_serv$Office_Full <- ifelse(is.na(def_serv$ContractingOfficeName), def_serv$Office, def_serv$ContractingOfficeName)
def_serv$Office_Full <- ifelse(def_serv$Office_Full == def_serv$Office, 
                                      def_serv$Office, 
                                      paste(def_serv$Office, 
                                            " - ", 
                                            def_serv$ContractingOfficeName))

grouped_barplot("Office_Full", def_serv,top_rank = 10)+
                  ggtitle("Percent of Frequency and obligation for Office")

grouped_barplot("Office_Full", def_serv,top_rank = 25)+
                  ggtitle("Percent of Frequency and obligation for Office")
...
centered_log_description(def_serv$l_def6_obl_lag1Const,"obligations")
...
freq_continuous_plot(def_serv,"cln_Def3HHI",bins=50, log=TRUE)
```
...
```{r Def2US, fig.width=7.5 , fig.height=3}
...
ggsave(NAICS3long_wide+theme(text=element_text(size=13)),file="..//Output\\Figure2.png",width=9.5,height=6,dpi=600)
ggsave(NAICS3long_wide+theme(text=element_text(size=13)),file="..//Output\\Figure2.eps",width=9.5,height=6,dpi=600)
ggsave(NAICS3long_wide+theme(text=element_text(size=13)),file="..//Output\\Figure2.svg",width=9.5,height=6,dpi=600)
...
NAICS_summary<-def_serv %>% group_by(NAICS6,StartCY,def6_HHI_lag1,def6_obl_lag1Const,def6_ratio_lag1,US6_avg_sal_lag1Const) %>%
  dplyr::summarise(annual_action_obligation=sum(Action_Obligation_OMB20_GDP18),
                   annual_count=length(StartCY)) 
...
NAICS_summary$principalnaicscode<-as.character(NAICS_summary$principalnaicscode)
NAICS_summary<-as.data.frame(NAICS_summary)
NAICS_summary$principalnaicscode[NAICS_summary$principalnaicscode==""]<-NA
...
levels(NAICS_long$variable)<- list("Herfindahl-\nHerschman Index"=c("def6_HHI_lag1"),
                                   "Defense Obligations"=c("def6_obl_lag1Const"),
...
statsummary_continuous(c("def6_ratio_lag1"), def_serv)
summary(rescale(na_non_positive_log(def_serv$def6_ratio_lag1,1)))
