...
$β_{11}cl\_Ceil\_Then\_Year_{i}~+~β_{12}cl\_Days_{i}~+~(~β_{13}SIDC_{i}~+~β_{14}MIDC_{i}+~β_{15}FSS–GWAC_{i}+~β_{16}BPA–BOA_{i}~)~+$

$(~β_{17}Other\_FP_{i}~+~β_{18}Incentive_{i}~+~β_{19}Comb–Other_{i}~+~β_{20}Other\_CB_{i}~+~β_{21}TM–LH–FPLOE_{i}~)~+$

$β_{22}b\_UCA_{i}~+~β_{23}b\_Intl_{i}~+~β_{24}cl\_def6\_HHI\_lag1l_{i}·cl\_def6\_obl\_lag1_{i}~+$

$β_{25}cl\_def6\_HHI\_lag1_{i}·b\_UCA_{i}~+~β_{26}cl\_Ceil\_Then\_Year_{i}·b\_UCA_{i}~+~(~β_{27}1Offr_{i}·b\_UCA_{i}~+$

$β_{28}2Offr_{i}·b\_UCA_{i}~+~β_{29}3–4Offr_{i}·b\_UCA_{i}+~β_{30}5plusOffr_{i}·b\_UCA_{i}~)~+~ε_{i}),$ 
...
$β_{1}cl\_def3\_HHI\_lag1_{i}~+~β_{2}cl\_def6\_HHI\_lag1_{i}~+~(~β_{3}1Offr~+~β_{4}2Offr~+$
...
$Probability~of~Termination~(y_i = 1) =$
...
$b\_Term~\sim~cl\_US6\_avg\_sal\_lag1Const~+$
$$cl\_CFTE~+~c\_pPBSC+c\_pOffPSC+$$
$$c\_pairHist+cl\_pairCA+$$
$$cl\_Ceil~+~cl\_Days+cl\_Base2Ceil+$$
$$Comp1or5+$$
$$Veh+$$
$$PricingUCA+$$
$$Crisis+$$
$$cl\_def6\_HHI\_lag1+cl\_def6\_ratio\_lag1+$$
$$cl\_def3\_HHI\_lag1+cl\_def3\_ratio\_lag1+$$
$$c\_pMarket+$$
$$cl\_OffVol+cl\_office\_naics\_hhi\_k,~family=binomial(link="logit"))$$
